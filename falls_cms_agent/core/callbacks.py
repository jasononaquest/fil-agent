...
async def emit_status(
    ...
    """
    if not Config.events_enabled() or not session_id:
...
def emit_status_sync(
    ...
    try:
        ...
        if loop.is_running():
            ...
            asyncio.create_task(emit_status(session_id, message, event_type, extra_data))
        else:
            loop.run_until_complete(emit_status(session_id, message, event_type, extra_data))
    except RuntimeError:
        # No event loop - create one
        asyncio.run(emit_status(session_id, message, event_type, extra_data))
...
def create_step_callback(
    ...
    def callback(callback_context: Any) -> None:
        # Try to get session_id from context if not provided
        sid = session_id
        if sid is None and hasattr(callback_context, "session"):
            sid = getattr(callback_context.session, "id", None)

        emit_status_sync(sid, f"{step_name}...", "step_start")
...
def create_complete_callback(
    ...
    def callback(callback_context: Any) -> None:
        ...
        emit_status_sync(sid, f"{step_name} complete", "step_complete")
